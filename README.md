...
| `ATENDIMENTO_URL` | Canal de atendimento da LP (página do atendimento AUVP), linkado no rodapé e abaixo do formulário | ✅ **Configurada.** Idem |
...
| `FAIXAS_REPASSE` | Tabela progressiva do repasse, da pergunta 9 da FAQ: a faixa é escolhida pela **receita bruta mensal** gerada na plataforma e o percentual vale sobre a receita inteira (não é alíquota marginal) | até R$ 10 mil: 50% · até R$ 30 mil: 55% · até R$ 50 mil: 60% · até R$ 80 mil: 65% · acima: 70% |
| `REPASSE_TETO` | Derivado da tabela (último percentual); é o "até 70%" que o resto da página cita | `0.7` |

O aviso ao lado dos números, a faixa exibida sob o valor e a barra de divisão (larguras, legenda e `aria-label`) são escritos a partir dessas constantes, então mudar a tabela mantém conta e texto alinhados.

> **O simulador mostra a faixa, e diz qual é.** Com o fee de referência de 1% ao ano, R$ 20 mi de custódia dão R$ 16,7 mil por mês, que caem na faixa de 55%; o teto de 70% só entra acima de R$ 96 mi. O rótulo do valor leva asterisco e o aviso abre com "* SIMULAÇÃO" em destaque, dentro do painel escuro, colado no número: não é letra miúda decorativa, é o que qualifica o resultado.
>
> **Histórico:** até setembro a LP comunicava só o teto, por decisão do comercial, e o simulador projetava 70% sobre tudo. A FAQ revisada publicou a tabela progressiva (pergunta 9), e o time pediu que a calculadora seguisse o mesmo racional; a tabela agora é pública na própria página, então não há mais o que reservar.
...
5. **O repasse é de até 70%** — teto da tabela progressiva (50% a 70%, pela receita mensal), publicada na FAQ e aplicada pelo simulador, que mostra a faixa em que a carteira cai e diz isso no asterisco.
...
7. **Dúvidas frequentes** — a dobra `#faq` é a **última antes do rodapé**, depois do formulário, e traz as 19 perguntas do documento *FAQ AUVP Advisor* (versão revisada) em acordeão nativo (`<details>`/`<summary>`, sem JS), agrupadas em **seis macrotemas**, como na FAQ da escola: 01 Posso entrar na AUVP Advisor? (perguntas 2, 7, 8, 15, 17), 02 Qual modelo de negócios e remuneração? (9, 10), 03 Como funciona minha carteira e o BTG? (1, 4, 16), 04 O que posso e o que não posso fazer? (3, 5, 6, 12), 05 Qual é minha relação com a AUVP e com meus clientes? (11, 13) e 06 Como funciona minha operação na plataforma? (14, 18, 19). A numeração é a do documento e está no comentário acima de cada `<details>`. **As notas internas do documento ficam de fora**, e as respostas 1 a 8 são as da versão revisada, que mudou várias delas (a 3, por exemplo, passou de "sim, há exclusividade" para "não"). Os temas viram abas por JS (`iniciarFaq`), numa **lista vertical discreta à esquerda das perguntas** (só texto, o escolhido em tinta cheia com um filete de acento; no celular a lista vira uma fileira acima das perguntas); sem JS ficam empilhados com título. Um tema sem pergunta pode ficar com `hidden` no botão e no painel, e o script o ignora. **A copy é reproduzida na íntegra, por decisão do time:** a primeira versão trocava "consultor" por *advisor* e "AUVP Advisor" por *a AUVP Advisors* seguindo as regras de vocabulário, e a correção enviada em seguida devolveu o texto original. É a única dobra em que essas duas regras não valem; ao revisar a FAQ, revise a partir do documento, não da página. Um ponto para o jurídico confirmar: a resposta sobre patrimônio fora do BTG (pergunta 4) **cita XP e Ágora pelo nome**, e a mesma regra de marca que vale para o BTG pode valer para elas. (A divergência antiga sobre ofertar outros produtos foi resolvida pela versão revisada: a pergunta 6 agora diz que o consultor não distribui nem é remunerado por eles, como a dobra de operação.)
...
- **O repasse é de até 70% em toda a página, e os 80% existem só como bônus de lançamento.** O número já mudou quatro vezes ao longo do projeto: 70% padrão com 80% para os 20 primeiros, depois "faixa de 70% a 80% para todos", depois padrão fixo de 70%, depois **até 70% com os 80% descartados**, e enfim **até 70% com os 80% de volta, agora por 12 meses e confinados à dobra dos benefícios de lançamento**. **Ao mexer no teto, mexa nos quatro lugares:** `FAIXAS_REPASSE` (o teto é o último percentual), a `meta description`, o benefício 01 e suas palavras-chave, e o aviso legal do rodapé; a barra de divisão e a legenda seguem a tabela sozinhas.
- **A calculadora parte do patrimônio sob custódia.** O controle anda de R$ 1 mi a R$ 250 mi (começando em R$ 20 mi); o fee anual sai daí por `TAXA_FEE_ANUAL`, a receita mensal escolhe a faixa em `FAIXAS_REPASSE` e o percentual da faixa vale sobre o fee inteiro. A barra de divisão é demonstrada sobre R$ 100 de fee, na faixa em vigor. A pergunta 10 da FAQ traz o fee por patrimônio **do cliente** (sugerido de 0,50% a 1,20%); como o controle é o patrimônio total do advisor, o simulador segue com o 1% de referência e diz isso no aviso.
