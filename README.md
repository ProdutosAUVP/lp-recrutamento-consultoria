...
6. **Verba de marketing** — o investimento em marketing para os primeiros advisors **não está na página**: segue em validação. Se for confirmado, entra como um bloco próprio e sem vínculo com ordem de chegada.
...
- **Alturas iguais dentro de uma fileira — quando as massas são comparáveis.** Nas cinco etapas da timeline isso vem de `align-items: stretch` na grade mais `flex: 1` no cartão, e os textos têm tamanho parecido. Na dobra escura vale o contrário: depois que o co-brand saiu, o primeiro cartão ficou com metade do conteúdo do segundo, e esticá-lo abria um vão de quase 200px entre o texto e a nota. Ali a grade usa `align-items: start`. **A regra que importa não é "mesma altura", é "sem espaço inventado":** se dois cartões têm massa parecida, iguale as alturas e ancore o último elemento com `margin-top: auto`; se um tem metade do outro, alinhe pelo topo
