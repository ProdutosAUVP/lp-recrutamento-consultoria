...
| `REPASSE_PADRAO` | Repasse do fee ao advisor, o espelho do comissionamento de 30% da AUVP | `0.7` (70%) |

A observação exibida abaixo do controle é escrita a partir dessas duas, então mudar um número mantém conta e texto alinhados. Os percentuais da legenda da barra de divisão (70% e 30%) estão no HTML e precisam ser ajustados junto.

> **Os 80% do benefício de lançamento não entram na calculadora, de propósito.** A conta usa só `REPASSE_PADRAO`, para a página nunca projetar receita sobre uma condição que depende de análise de elegibilidade e que nem todo mundo terá. O número vive apenas no bloco `.verba`, ao lado do selo que diz isso.
...
5. **O repasse é um número só: 70%** — não há faixa nem critério a definir. O comissionamento da AUVP é de 30% e o do advisor, de 70%, e é isso que a calculadora projeta. Os 80% existem apenas como benefício de lançamento, sujeito a análise de elegibilidade, e ficam fora da conta de propósito.
6. **Benefícios de lançamento** — o bloco com os dois benefícios dos primeiros advisors aprovados (comissão fixa de 80% de repasse e R$ 10.000 em verba de marketing) fica logo abaixo da calculadora, e ambos aparecem no aviso legal do rodapé. **Cada item carrega um selo de "sujeito a análise de elegibilidade"** no próprio cartão, além da letra miúda que fecha o bloco: nenhum dos dois é automático por ordem de chegada. **Confirmar antes de publicar:** a última informação registrada era de que a verba de marketing seguia em validação, e os critérios de elegibilidade nunca foram definidos, então a página fala em "primeiros advisors" sem cravar quantidade. Se um dos benefícios cair, o item sai daqui e do aviso legal ao mesmo tempo, para não sobrar promessa órfã no rodapé.
...
- **O repasse padrão é 70%, e os 80% são benefício de lançamento.** É o espelho do comissionamento de 30% da AUVP declarado no escopo. Esse modelo já foi outro duas vezes: virou "faixa de 70% a 80% para todos" quando a orientação foi que o fee poderia chegar a 80%, e voltou ao padrão fixo de 70% quando ficou claro que o teto geral é 70%. **Ao mexer no número, mexa nos seis lugares:** `REPASSE_PADRAO`, a `meta description`, o benefício 01 e suas palavras-chave, a legenda da barra de divisão, o texto do item de 80% no bloco `.verba` e o aviso legal do rodapé.
- **A calculadora parte do patrimônio sob custódia.** O controle anda de R$ 1 mi a R$ 250 mi (começando em R$ 20 mi); o fee anual sai daí por `TAXA_FEE_ANUAL` e o repasse, por `REPASSE_PADRAO`. A barra de divisão é demonstrada sobre R$ 100 de fee.
...
- **Compliance:** repasse padrão de 70% declarado como definido em contrato, comissionamento da AUVP de 30% declarado junto à calculadora, benefícios de lançamento marcados item a item como sujeitos a análise de elegibilidade, premissa de fee exposta abaixo do controle, vinculação ao BTG sinalizada como sujeita ao compliance da instituição e sem nenhuma aplicação da marca deles, suporte à regularização diferenciado de garantia de registro, e disclaimers da Resolução CVM 19/2021 no hero, no formulário e no rodapé
